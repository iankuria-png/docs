...
No one can effectively sell, support, market, or improve a product they don't fully understand. The Exotic Online University gives every team member — Sales & CS, Finance, RD Team & IT Infrastructure, and SEO & Marketing — a shared, accurate view of how the platform works from the **escort's perspective**.
...
<Warning>
  Package tiers, features, and pricing are **owned by RD** within the combined RD Team & IT Infrastructure function. If an escort asks you to change a price, create a new tier, or grant a discount beyond the approved threshold — escalate to your manager. Do not treat pricing as an SEO & Marketing or Sales decision.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Finance — what you own" icon="calculator">
    | Stage | Responsibility |
    | --- | --- |
    | Payment confirmation | **Full ownership** — weekly reconciliation, manual review queue, reversals |
    | Settlement review | Approve/reject amount-mismatch payments (±5% tolerance) |
    | Client resolution | Match payments to clients when auto-match fails (during review) |
    | Renewal revenue | Track renewal MRR |
    | Discounts | Review deals and flag any suspicious payment that needs review |
  </Accordion>

  <Accordion title="RD Team & IT Infrastructure — what you own" icon="lightbulb">
    | Area | Responsibility |
    |---|---|
    ...
    | Payment infrastructure | Provider routing, webhooks, gateway health, and `BillingMarketProviderBinding` config |
    | WordPress sync | `WpSyncService` health, activation endpoint, plugin maintenance |
    | Tracking setup | Google Analytics tag setup, contact-tap instrumentation, and platform event wiring |
    | Monitoring | `laravel.log`, webhook dashboards, scheduled tasks, and activation recovery |
  </Accordion>
  ...
  <Accordion title="SEO & Marketing — what you own" icon="chart-line">
    ...
    | Performance monitoring | Flag drop-offs in registrations, payments, and contact engagement back to RD Team & IT Infrastructure |
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="RD Team & IT Infrastructure" icon="lightbulb" href="/product/stage-5a-subscription-selection">
    Start at Stage 5a — Subscription selection, then 5b, 5c, 5d, and 5e to understand pricing, routing, activation, and failure recovery.
  </Card>
